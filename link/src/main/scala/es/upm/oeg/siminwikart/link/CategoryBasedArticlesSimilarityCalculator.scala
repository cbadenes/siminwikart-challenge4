...
class CategoryBasedArticlesSimilarityCalculator {
  ...
  def calculateSimilarities(linkSet1:Set[String], linkSet2:Set[String]) : Double = {
    ...
    val arriva:Double = intersectionSet.size;
    val abajo:Double = ((linkSet1.size + linkSet2.size) / 2);
    val similarityValue = arriva / abajo;
    similarityValue;
  }
  
  def calculateSimilarities(articlesLinks:RDD[Array[String]]
  , article1ID:String, article2ID:String) : Double = {
...
//  def apply (corpus: RDD[RegularResource], topics: Integer, alpha: Double, beta: Double, maxIt: Integer): (ConceptsSpace,TopicsSpace)= {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val articlesToTest : Set[String] = Set("507433","240390","2484564","2150841");
    ...
    val articlesLinks = sc.textFile(articlesLinksFileURL).
      map(_.replace("\"","").split(",")).filter(_.size>1).filter(x=>articlesToTest.contains(x(0))||articlesToTest.contains(x(1)) )
...
//    println("linksSeveroOchoaSet.size = " + linksSeveroOchoaSet.size);
    ...
    val combination2OfArticlesToTest:List[Set[String]] = articlesToTest.subsets(2).toList;
    println("combination2OfArticlesToTest = " + combination2OfArticlesToTest.toList);
    combination2OfArticlesToTest.toList.map(x => {
      val xlist = x.toList;
      println("calculating sim(" + xlist(0) + ","+ xlist(1) + ") ...");
      val similarityValue = testObject.calculateSimilarities(articlesLinks
        , xlist(0), xlist(1));
      println("sim(" + xlist(0) + ","+ xlist(1) + ") = " + similarityValue);
    });
